...
// Chart legend — intentionally a hand-built 2-color + hash legend
// (NOT the 4-color canonical RangeStateBadge palette). On the small
// per-ticker chart, two strongly readable bull/bear fills + a hashed
// compression/expansion marker scan faster than four distinct hues.
// Other surfaces (RangeStateBadge, ambient backdrop) still use the
// canonical 4-color token — chart-only divergence is deliberate.
    ...
    <div className="chart-wrap">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={closeData} margin={{ top: 8, right: 8, bottom: 4, left: 0 }}>
          <defs>
            {/* Pattern strokes use the bull/bear base colors so the
                hatched compression/expansion fills read as "directional
                family, non-directional state" — green hash for LH/HL
                compression, red hash for HH/LL expansion. Matches the
                chart legend swatches. Chart-only convention; the
                canonical 4-color range_state token is used elsewhere. */}
